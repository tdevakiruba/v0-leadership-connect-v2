...
export async function POST(request: Request) {
  ...
  console.log("[v0] Stripe webhook received at /api/stripe/webhook")
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      if (session.payment_status === "paid") {
        ...
        console.log("[v0] Payment successful - userId:", userId, "durationDays:", durationDays, "productId:", productId)
        ...
        if (userId) {
          ...
          console.log("[v0] Subscription dates - start:", startDate.toISOString(), "end:", endDate.toISOString())
    ...
    case "checkout.session.expired": {
      console.log("[v0] Checkout session expired:", event.data.object.id)
      break
    }
