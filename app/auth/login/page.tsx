...
export default function LoginPage() {
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    if (data.user) {
      ...
      if (!hasAgreement) {
        // Record the IP agreement (user implicitly agrees by signing in)
        try {
  ...
  // Show statement for OAuth (always required) or if user has no existing agreement
  const showIPStatement = hasExistingAgreement === false || hasExistingAgreement === null
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-primary relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-primary-foreground">
              ...
              {['S', 'I', 'G', 'N', 'A', 'L'].map((letter, i) => (
                <div 
                  ...
                  className={`aspect-square rounded-lg flex items-center justify-center text-lg font-bold ${
                    i < 5 ? 'bg-white/10 text-primary-foreground' : 'bg-accent text-accent-foreground'
                  }`}
                >
                  {letter}
                </div>
          ...
          <p className="text-primary-foreground/50 text-sm">
            Trusted by senior leaders at Fortune 500 companies
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <Card className="border-0 shadow-xl">
            ...
            <CardContent className="space-y-6">
              {/* IP Agreement Statement - Only show if user hasn't agreed */}
              {showIPStatement && (
